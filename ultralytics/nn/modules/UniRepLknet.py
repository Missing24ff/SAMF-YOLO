...
class UniRepLKNet(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        dims = tuple([int(dim * factor) for dim in dims])
        # depths = [max(round(dim * depth_factor), 1) if dim > 1 else dim for dim in depths]
        # n = n_ = max(round(dim * depth_factor), 1) if dim > 1 else dim  # depth gain
        depths = tuple(depths)
...
def unireplknet_a(factor, pretrained='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_A_F_P_depths, dims=(40, 80, 160, 320), **kwargs)
    if pretrained:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(pretrained)))
    return model

def unireplknet_f(factor, pretrained='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_A_F_P_depths, dims=(48, 96, 192, 384), **kwargs)
    if pretrained:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(pretrained)))
    return model

def unireplknet_p(factor,pretrained='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_A_F_P_depths, dims=(64, 128, 256, 512), **kwargs)
    if pretrained:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(pretrained)))
    return model

def unireplknet_n(factor, weights='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_N_depths, dims=(80, 160, 320, 640), **kwargs)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)))
    return model

def unireplknet_t(factor, pretrained='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_T_depths, dims=(80, 160, 320, 640), **kwargs)
    if pretrained:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(pretrained)))
    return model

def unireplknet_s(factor, pretrained='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_S_B_L_XL_depths, dims=(96, 192, 384, 768), **kwargs)
    if pretrained:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(pretrained)))
    return model

def unireplknet_b(factor, pretrained='', **kwargs):
    model = UniRepLKNet(factor, depths=UniRepLKNet_S_B_L_XL_depths, dims=(128, 256, 512, 1024), **kwargs)
    if pretrained:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(pretrained)))
    return model
